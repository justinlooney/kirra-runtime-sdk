...
class AegisFirewallNode : public rclcpp::Node {
public:
    AegisFirewallNode() : Node("aegis_firewall_node") {
        ...
        safe_publisher_     = this->create_publisher<geometry_msgs::msg::Twist>("/aegis/cmd_vel_safe", qos_volatile);
        state_publisher_    = this->create_publisher<std_msgs::msg::String>("/aegis/system_state", qos_transient);
        audit_publisher_    = this->create_publisher<std_msgs::msg::String>("/aegis/audit_events", qos_volatile);
        trust_publisher_    = this->create_publisher<std_msgs::msg::UInt32>("/aegis/metrics/trust_score", qos_volatile);
        mutation_publisher_ = this->create_publisher<std_msgs::msg::UInt32>("/aegis/metrics/mutation_count", qos_volatile);

        raw_subscriber_ = this->create_subscription<geometry_msgs::msg::Twist>(
            "/cmd_vel",
            qos_volatile,
            std::bind(&AegisFirewallNode::handle_incoming_trajectory, this, std::placeholders::_1)
        );
...
private:
    void handle_incoming_trajectory(const geometry_msgs::msg::Twist::SharedPtr raw_msg) {
        ...
        uint32_t active_trust = aegis_get_trust_score();
        ...
        double sanitized_linear_x  = aegis_filter_move_velocity(raw_msg->linear.x, DT);
        double sanitized_angular_z = aegis_filter_rotate_velocity(raw_msg->angular.z, DT);
        uint32_t post_filter_trust = aegis_get_trust_score();
    ...
    void force_hard_lockout(double raw_val) {
        ...
        safe_publisher_->publish(safe_msg);
        publish_audit_log("HARD_FAILSAFE_ACTIVE", raw_val, 0.0, aegis_get_trust_score(), "CRITICAL_LOCKOUT");
    }
    ...
    void publish_audit_log(const std::string & e, double r, double s, uint32_t t, const std::string & m) {
        auto msg = std_msgs::msg::String();
        msg.data = "[AEGIS AUDIT] EVENT=" + e + " | RAW=" + std::to_string(r)
...
int main(int argc, char **argv) {
    ...
    rclcpp::spin(std::make_shared<AegisFirewallNode>());
    rclcpp::shutdown();
    return 0;
}
