#!/usr/bin/env python3
"""
Aegis Posture SSE Bridge

Connects to the Aegis SSE posture stream and republishes posture transitions
as ROS2 topics. Also emits an emergency stop when posture transitions to LockedOut.

SSE source: GET /system/posture/stream
ROS2 outputs:
  /aegis/fleet_posture   (std_msgs/String) — "Nominal" | "Degraded" | "LockedOut"
  /aegis/posture_events  (std_msgs/String) — JSON event stream
"""
...
class PostureSubscriber(Node):
    def __init__(self):
        ...
        self.declare_parameter('aegis_client_id', 'ros2-posture-bridge-01')
        ...
        self._pub_posture = self.create_publisher(String, '/aegis/fleet_posture', 10)
        self._pub_events = self.create_publisher(String, '/aegis/posture_events', 10)
        ...
        self.get_logger().info(
            f'Aegis posture bridge started (SSE: {self._aegis_url}/system/posture/stream)'
        )
    ...
    def _sse_loop(self):
        ...
        url = f'{self._aegis_url}/system/posture/stream'
        while self._running:
            try:
                self.get_logger().info(f'Connecting to Aegis SSE stream: {url}')
                with requests.get(url, headers=self._headers(), stream=True, timeout=30) as resp:
                    ...
                    self.get_logger().info('Connected to Aegis SSE stream.')
                    buffer = ''
                ...
                self.get_logger().error(
                    f'Cannot reach Aegis SSE stream at {url}. '
