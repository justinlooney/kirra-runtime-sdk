#!/usr/bin/env python3
"""
Aegis Sensor Health Monitor

Subscribes to robot sensor topics and posts health reports to Aegis.
Maps ROS2 sensor data freshness and quality to Aegis fleet node confidence scores.
...
class SensorMonitor(Node):
    def __init__(self):
        ...
        self.declare_parameter('aegis_client_id', 'ros2-sensor-monitor-01')
        ...
        self.get_logger().info(
            f'Aegis sensor monitor started (report_interval={self._report_interval_s*1000:.0f}ms)'
        )
    ...
    def _post_report(self, node_id: str, confidence: float, hardware_fault: bool, timestamp_ms: int):
        ...
        try:
            resp = requests.post(
                f'{self._aegis_url}/fleet/diagnostics/report',
            ...
            if resp.status_code not in (200, 201):
                self.get_logger().debug(
                    f'Aegis diagnostics report for {node_id} returned {resp.status_code}'
                )
        except requests.Timeout:
            self.get_logger().debug(f'Aegis diagnostics timeout for {node_id}')
        except requests.ConnectionError:
            pass  # Aegis unreachable -- silently skip, will retry next interval
