...
class CmdVelInterceptor(Node):
    def __init__(self):
        ...
        # Parameters
        self.declare_parameter('aegis_url', 'http://localhost:8090')
        self.declare_parameter('aegis_token', '')
        self.declare_parameter('aegis_client_id', 'ros2-interceptor-01')
        ...
        self.declare_parameter('enforcement_topic', '/aegis/enforcement_action')
        self.declare_parameter('posture_topic', '/aegis/fleet_posture')
        self.declare_parameter('timeout_ms', 50)
        self.declare_parameter('fallback_on_timeout', 'stop')

        self._aegis_url = self.get_parameter('aegis_url').value
        self._aegis_token = self.get_parameter('aegis_token').value
        self._client_id = self.get_parameter('aegis_client_id').value
        ...
        self.get_logger().info(
            f'Aegis cmd_vel interceptor started: '
            f'{input_topic} -> Aegis -> {output_topic} '
            f'(fallback={self._fallback}, timeout={self._timeout_s*1000:.0f}ms)'
        )

    def _headers(self):
        return {
            'Authorization': f'Bearer {self._aegis_token}',
            'x-aegis-client-id': self._client_id,
            'Content-Type': 'application/json',
        }

    def _twist_to_proposed_command(self, twist: Twist) -> dict:
        """
        Convert a ROS2 Twist message to a ProposedVehicleCommand for Aegis.
    ...
    def _on_cmd_vel(self, msg: Twist):
        ...
        try:
            resp = requests.post(
                f'{self._aegis_url}/actuator/motion/command',
            ...
            if resp.status_code == 200:
                ...
                # Fleet locked out or Aegis down -- fail closed
                ...
                self.get_logger().warn(
                    f'Aegis blocked command: HTTP {resp.status_code} posture={posture}'
                )
            else:
                self._publish_stop(f'HTTP_{resp.status_code}')
                self.get_logger().error(f'Aegis returned unexpected status: {resp.status_code}')
            ...
            self.get_logger().error(f'Unexpected error in Aegis enforcement: {e}')
            self._publish_stop('UNEXPECTED_ERROR')

    def _handle_timeout(self, original_msg: Twist, proposed: dict):
        self.get_logger().warn(
            f'Aegis enforcement timeout ({self._timeout_s*1000:.0f}ms). '
    ...
    def _handle_connection_error(self, original_msg: Twist):
        self.get_logger().error(
            f'Cannot reach Aegis at {self._aegis_url}. '
            f'Fallback: {self._fallback}.'
        )
        if self._fallback == 'passthrough':
            self._pub_safe.publish(original_msg)
    ...
    def _build_safe_twist(self, original: Twist, enforced_v: float, enforced_s_deg: float) -> Twist:
        """
        Reconstruct a safe Twist from the Aegis-enforced speed and steering.
