...
struct MotionCommandResponse {
    /// The post-enforcement command (may differ from request if clamped)
    linear_velocity_mps: f64,
    steering_angle_deg: f64,
    /// "Allow", "ClampLinear", "ClampSteering", "DenyBreach"
    enforcement_action: String,
    /// Populated when enforcement_action is "DenyBreach"
    #[serde(default)]
...
enum ScenarioEvent {
    ...
    /// Request a dangerous command (should be clamped)
...
struct Scenario {
    name: String,
    events: Vec<(u64, ScenarioEvent)>, // (elapsed_ms, event)
    duration_ms: u64,
}
