# Aegis — Project Knowledge Base

*Paste this document into your Claude project's knowledge base or custom instructions to give any conversation in that project full context about the Aegis system.*
...
**The core problem it solves**: When an AI agent or LLM is in the loop controlling physical systems (robots, drones, industrial machinery), you cannot trust that every output it generates is safe or authorized. Aegis sits between the AI and the actuators as a cryptographically grounded gatekeeper that continuously evaluates the trust state of the entire fleet before letting any command through.
...
A Tower middleware service (`AegisPolicyLayer`) that intercepts all control traffic. Commands are classified into four bins:
...
Multiple Aegis controllers can share trust observations across organizational boundaries. Each federated trust report must:
...
**Auth tiers**:
- **Identity-gated routes** (posture stream, federation submit, action filter, industrial evaluate): require admin Bearer token AND a valid `x-aegis-client-id` header — enforces that requests arrive through an authorized mesh proxy, not a raw caller who obtained the token
...
- `AEGIS_TRUSTED_INGRESS_MODE` (`true`/`1` to enable) — fail-closed: disabled by default
- `AEGIS_CLIENT_ID_HEADER` (default: `x-aegis-client-id`)
...
- **PassiveStandby** (`AEGIS_VERIFIER_MODE=passive`): Mutation routes return 503 — HA hot-spare that prevents split-brain writes
...
1. AegisKernelGovernor — scalar clamping and rate limiting
...
7. Add hardcoded fallbacks for `AEGIS_ADMIN_TOKEN` or `AEGIS_SUPERVISOR_RESET_KEY`
