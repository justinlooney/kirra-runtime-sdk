# Apollo + Kirra Integration — Architecture Sketch and Execution Plan

**Status:** Pre-execution sketch. This integration is sequenced after the Kirra silicon matrix increment (Increment 4) and the ROS2 bring-up (PARK-036). Current active work: PARK-001/002/003 (runtime core). Do not start Apollo work until QNX (PARK-024, TIME-SENSITIVE) and the ROS2 interlock demo (PARK-037) are complete.
...
```
Planning → Control → [Kirra Governor] → Canbus → Vehicle
                          │
                          └─ Posture / Audit / Federation
```

**Why this is the right first cut:**
- Control's output is a concrete `ControlCommand` (throttle %, brake %, steering angle, gear). Kirra already evaluates exactly this kind of command.
- Insertion is *transparent* to Apollo — Control still publishes its commands on `/apollo/control`, but Canbus subscribes to `/apollo/control/validated` (the Kirra output topic) instead of the original.
- If Kirra goes offline or fails open, you can revert by re-pointing Canbus at the original topic in the Cyber RT config. Operational rollback is one config line.
...
```
Prediction → Planning → [Kirra Trajectory Filter] → Control → Canbus → Vehicle
```

**Why not first:**
- Planning's output is a trajectory (a sequence of (x, y, t, v) waypoints), not a single command. Kirra would need to evaluate the trajectory as a whole — every waypoint must satisfy the envelope, plus inter-waypoint feasibility.
- This is genuinely useful — it catches unsafe *plans* before they become unsafe *commands* — but it's a deeper integration and a different kind of evaluation than Kirra currently does.
- Save it for v2.

### Option C (mentioned for completeness, do not pursue)

Replacing or modifying Apollo's perception or prediction outputs. This is out of scope. Kirra is not a perception system.
...
└────────────────────────────────────────────┼─────────────────────────┘
              ...
              └─────────────────────────────────────────────────┘
                                             │
                                             ▼
                          /apollo/control/kirra_validated
                                             │
                                             │ Cyber RT subscription
                                             ▼
              ┌─────────────────────────────────────────────────┐
...
```
              ┌─────────────────────────────────────────────────┐
              │              Kirra Governor State               │
              └──────────────┬───────────────┬──────────────────┘
                             │               │
                             ▼               ▼
              /apollo/kirra/posture   /apollo/kirra/audit
                             │               │
                             ▼               ▼
           ┌────────────────────┐   ┌─────────────────────┐
           │  Apollo HMI /      │   │  Kirra audit chain  │
           │  Dreamview overlay │   │  → flash/disk write │
           │  shows lockout     │   │  → SSE export       │
           └────────────────────┘   └─────────────────────┘
```

The **posture topic** carries `FleetPosture` (Nominal / Degraded / LockedOut) and reason codes. Apollo's monitor module subscribes and exposes the state in Dreamview, so an operator sees *why* Kirra intervened.

The **audit topic** is the tamper-evident chain. It's not consumed by Apollo — it's exported via the existing Kirra audit chain mechanisms to whatever storage backend you're using.
...
### Phase 1: Kirra bridge skeleton (3-5 days)

Goals: a Rust/C++ process that subscribes to `/apollo/control`, deserializes it, prints it, and republishes unchanged to `/apollo/control/kirra_validated`. No actual safety logic yet.

Tasks:
1. Decide language. Apollo's native idiom is C++; Cyber RT has C++ APIs first. Rust bindings exist (third-party, search for `cyber_rs` and similar) but are less mature. **Recommendation: thin C++ shim that talks to Kirra over a local IPC, with Kirra itself remaining in Rust.** This keeps Kirra's core unchanged and isolates Apollo-specific dependencies.
...
### Phase 2: Kirra evaluation hook (5-7 days)

Goals: actual safety logic in the loop. Bridge submits each ControlCommand to Kirra, Kirra evaluates, bridge respects the result.

Tasks:
1. Define the mapping from `ControlCommand` (throttle/brake/steering) to Kirra's `OperationalCommand::WriteState`. This is a vehicle-model-specific transform — throttle % → acceleration target, steering angle → angular velocity command, etc. Get this wrong and Kirra evaluates against incorrect physics.
2. Wire the bridge to call Kirra's evaluation API (via IPC or, if you decide on a single-process design, direct Rust FFI)
...
4. Add the posture publisher on `/apollo/kirra/posture`
5. Add the audit publisher on `/apollo/kirra/audit`

**Done when:** in simulation, an injected over-envelope command (e.g., 100% throttle from a stop while in reverse) gets rejected by Kirra and you see the lockout in Dreamview.

### Phase 3: Demo scenarios (3-5 days)

Goals: a small library of failure-mode scenarios that show what Apollo does versus what Apollo + Kirra does.

Scenario ideas (these are sketches; refine when you have Apollo running):

1. **Hallucinated lane change at speed.** Force Planning to produce a trajectory that demands lateral acceleration above ~4 m/s² at 60 km/h. Apollo alone: vehicle skids or rolls. Apollo + Kirra: command clamped to safe lateral g, gentle lane change, posture stays Nominal.

2. **Post-collision behavior.** Inject a "collision detected" signal (Apollo has crash detection in its monitor module). Apollo alone with default policy: pullover maneuver executes. Apollo + Kirra configured with a conservative post-collision policy: lockout asserted, no movement until supervisor reset. *Direct reference to the Cruise 2023 incident.*

3. **Sensor degradation cascade.** Disable perception's lidar fusion mid-drive. Apollo alone: planner gets degraded inputs and may produce erratic commands. Apollo + Kirra: posture engine detects sensor fault propagation via DAG, transitions to Degraded, restricts to ReadTelemetry, vehicle comes to a controlled stop.

4. **AI command flooding.** Apollo's planner stuck in a loop producing 1000 commands/sec. Apollo alone: Canbus gets overwhelmed or commands get out of phase. Apollo + Kirra: bounded ring buffer, rate limiting at the governor boundary.

Each scenario gets:
- A scripted Cyber RT recording or Dreamview replay
- Comparative video (Apollo alone vs. Apollo + Kirra)
- A short analysis: what Apollo did, what Kirra allowed, why
...
**Determinism in simulation.** Apollo's simulator is mostly deterministic but not perfectly. Comparative demos (Apollo alone vs. Apollo + Kirra) should use Cyber RT *recordings* as inputs, not live planning, to ensure the input stream is identical across runs. Otherwise you can't tell whether differences are from Kirra or from planner non-determinism.

**Kinematic model mismatch.** Kirra's existing kinematic envelope is bicycle-model-based with parameters for some asset types. Apollo's default vehicle is the Lincoln MKZ; its physical limits may differ from Kirra's defaults. Calibrate the envelope to the Apollo vehicle before running scenarios, or scenarios will fire incorrectly.

**Apollo's own safety modules.** Apollo has a `guardian` module that does some safety filtering already. Two-layer safety filtering is fine in principle but creates ambiguity in scenarios — which layer rejected what? Either disable guardian for the demo (cleaner story) or explicitly characterize Kirra as complementary to guardian (more nuanced story). Pick one and be consistent.

**Latency budget.** The bridge adds at least one IPC hop on the control path. Apollo's control loop targets ~100Hz (10ms cycle). The Kirra evaluation must complete in well under 10ms or you've broken Apollo's control loop. Benchmark this before scenario work. If latency is a problem, consider co-locating Kirra evaluation inside the bridge process (Rust → C++ FFI) rather than IPC.

**The post-collision scenario specifically.** Referencing the Cruise incident directly is powerful but politically charged. Some readers will see "Kirra would have saved her" as overclaiming. Frame it as: "Independent runtime safety governance with a conservative post-collision policy would have prevented the dragging maneuver. The policy is the operator's choice; Kirra is the mechanism for enforcing it independently of the AV stack." Don't claim Kirra prevents collisions. Claim Kirra enforces policy independently.

---

## 6. Writeup outline

Title (working): **Independent Runtime Safety Governance for End-to-End AV Stacks: An Apollo + Kirra Integration**
...
2. **What Kirra is** (300 words)
   - One-paragraph product description
   - The architectural role: between AI planner and physical actuator
   - What Kirra does *not* do (perception, planning, prediction — defer to the AV stack)
...
5. **Demo scenarios** (1000-1500 words, the meat of the post)
   ...
   - For each: setup, what Apollo alone does, what Apollo + Kirra does, why the difference matters
...
6. **Honest limitations** (400 words)
   ...
   - Kirra cannot prevent collisions, only enforce policy after a collision is detected
...
8. **Call to action** (100 words)
   ...
   - Invite Apollo and AV teams to evaluate Kirra on their stack
...
| Phase 2: Kirra evaluation hook | Week 2-3 |
...
1. A public GitHub repo with the Apollo + Kirra bridge code (Apache 2 license to match Apollo)
2. A configuration patch document showing how to integrate Kirra into an Apollo deployment
...
6. Concrete reference to point at in WeRide / partnership conversations: "here's Kirra integrated with Apollo, here's what it caught, here's what it didn't"
...
Do not start this while QNX is on the 30-day clock. The PARK-NNN order is:

1. **PARK-024** — QNX deployment spike (TIME-SENSITIVE — 30-day license)
2. **PARK-001–006** — Increment 1: Deterministic Runtime Core complete (parko-core v0.1.0 tagged)
3. **PARK-007–012** — Increment 2: HAL complete (InferenceBackend finalized, CPU ONNX verified)
4. **PARK-020–023** — TensorRT MVP on Jetson (parallel track to Increment 2)
5. **PARK-036, PARK-037** — Robot arrives → ROS2 bring-up → cmd_vel interlock demo
6. *Then* this Apollo work

If something is going wrong in (1)–(5) and you find yourself wanting to context-switch into Apollo because Apollo is more fun than QNX, **that is a signal, not a plan.** Apollo is the right next move *after* the core runtime, silicon matrix, and ROS2 work is done, not as an escape from it.
