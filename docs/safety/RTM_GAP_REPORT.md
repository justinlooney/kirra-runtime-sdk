...
| SG-012 | B | **CLOSED (Option C)** | Mechanism added (the prior gap — `Dnp3Adapter::evaluate` is a pure classifier — is bridged at the verifier-service handler). `src/bin/kirra_verifier_service.rs::evaluate_dnp3_adapter` (and the unified `evaluate_unified_industrial_request` path): every broadcast (`dest_address == DNP3_BROADCAST_ADDRESS`) is written to the tamper-evident chain via `save_posture_event_chained` BEFORE the verdict is returned; an audit-write failure (incl. poisoned store) **BLOCKS** the command — `allowed:false` / `DNP3_BROADCAST_AUDIT_UNAVAILABLE` / 503 (TR-012a). Unicast control is audited but non-fatal on failure (TR-012b). Tests: `test_dnp3_broadcast_always_audited`, `_blocked_on_audit_write_failure`, `_unicast_audit_failure_non_fatal`. **AoU:** Kirra CLASSIFIES, it does not actuate — "before *physical* control output" is the integrator's ordering obligation (the integrator MUST NOT actuate ahead of this audited verdict). |
